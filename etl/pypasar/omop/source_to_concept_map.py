...
class source_to_concept_map:
    ...
    def __init__(self):
        ...
        self.source_file = os.path.join(os.getenv("BASE_PATH"), "source_to_concep_map", "v0.1", "data.csv")
    ...
    def ingest(self):
        with self.engine.connect() as connection:
            with connection.begin():
                ...
                connection.execute(text(f"INSERT INTO source_to_concept_map (source_code, source_concept_id,source_vocabulary_id,source_code_description, target_concept_id, target_vocabulary_id, invalid_reason) SELECT t.source_code, t.source_concept_id, t.source_vocabulary_id, t.source_code_description, t.target_concept_id, c.vocabulary_id, c.invalid_reason FROM {self.temp_table} t INNER JOIN concept c ON c.concept_id = t.target_concept_id"))
        pass
