    ...
    def initialize(self):
        with self.engine.connect() as connection:
            with connection.begin():
                connection.execute(text(f'DELETE FROM {self.omop_schema}.concept_ancestor'))

    def process(self):
        connection = self.engine.raw_connection()
        with connection.cursor() as cursor:
            try:
                cursor.copy_expert(f"COPY {self.omop_schema}.concept_ancestor FROM STDIN WITH DELIMITER '\t' CSV HEADER QUOTE '\"' ESCAPE E'\\\\'" , open(self.source_file, "r", buffering=2**10))
                connection.commit()
