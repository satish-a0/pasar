...
class measurement():
    ...
    def fetch_in_batch_source_table(self, source_table_cols):
        ...
        # select_sql += f" FROM {source_table_cols['table']} order by anon_case_no LIMIT 1"
        #print(select_sql)
        with self.engine.connect() as connection:
            with connection.begin():
    ...
    def transform_preop_char(self, source_table_cols, source_batch, measurement_df):
        ...
        # combined_list = [source_batch["height"], source_batch["weight"], source_batch["bmi"],
        #                                           source_batch["systolic_bp"], source_batch["diastolic_bp"], source_batch["heart_rate"], source_batch["o2_saturation"], source_batch["o2_supplementaries"], source_batch["temperature"], source_batch["pain_score"]]
        # combined_list= [1,2,3,4,5,6,7]
        # 1 to Many
        if len(source_batch) > 0:
            measurement_df["measurement_date"] = pd.to_datetime(source_batch["session_startdate"]).dt.date
            measurement_df["measurement_type_concept_id"] = 32879
            # measurement_df["measurement_source_value"] = source_batch[""]
            value_as_number_df = source_batch[["height","weight","bmi", "systolic_bp", "diastolic_bp", "heart_rate", "o2_saturation", "o2_supplementaries", "temperature", "pain_score"]]
            # measurement_df["anon_case_no"] = source_batch["anon_case_no"]
        ...
        new_measurement_df = measurement_df.explode("value_as_number")
        new_measurement_df["measurement_id"] = range(self.measurement_id_start, (self.measurement_id_start + len(new_measurement_df)))
        print(new_measurement_df.head(len(new_measurement_df)))
        return new_measurement_df
